...
def plot_state_compare_age(data,data1):
    ...
    for i in range(3):
        axes[1][0].set_ylabel('感染人数', fontsize=14)
        for j in range(6):
            ...
            max = np.max(data[(i+1)*(j+1)])
            max1 = np.max(data1[(i+1)*(j+1)])
            axes[i][j].axhline(y=max, color='coral', linestyle='--',linewidth=0.5)
            axes[i][j].axhline(y=max1, linestyle='--',linewidth=0.5)
            axes[i][j].tick_params(labelsize=10)
            axes[i][j].plot(data[0][0:100], data[(i+1)*(j+1)][0:100],label='无症状感染者',color='coral')
            axes[i][j].plot(data1[0][0:100], data1[(i+1)*(j+1)][0:100],label='有症状感染者')
    ...
    plt.show()
    file_name = f"V_I_incidence_perday.png"
...
data = read_state_age('asymptomaticV')
data1 =read_state_age('infectedV')+read_state_age('infectedN')

#因为这个ndarray的第一列是列号，两个相加默认把列号相加了，所以得把第一列恢复成原来的列号
data = data.T
data1 = data1.T
data1[0]=data[0]

plot_state_compare_age(data, data1)