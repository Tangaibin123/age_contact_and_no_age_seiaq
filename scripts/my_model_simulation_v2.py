...
# %% 模型的模拟
def write_param_value(a, b1, b2, betaN, betaV, sigmaN_inverse, sigmaV_inverse, alpha_inverse, C, result_file_path):
    ...
    betaN = str(betaN)
    betaV = str(betaV)
    sigmaN_inverse = str(sigmaN_inverse)
    sigmaV_inverse = str(sigmaV_inverse)
    alpha_inverse = str(alpha_inverse)
    C = str(C)
    file_path = os.path.join(datadir, 'param_run_log_v2.csv')
    e = os.path.exists(file_path)
    if e:
        ...
        sWrite.writerow([date, a, b1, b2, betaN, betaV, sigmaN_inverse, sigmaV_inverse, alpha_inverse, C, result_file_path])
        file.close()
    else:
...
def dou_seaiq_with_age_specific_contact_martix(contact_matrix_arr, ages, a, w, k, beta, susceptibility_factor_vector,
                                               b1, b2, sigma_inverse, alpha_inverse, allocation_capacity, mode,
    ...
    for t in range(timesteps):
        ...
        for i in range(num_agebrackets):
            for j in range(num_agebrackets):
                incidenceN[i][t] += a * beta * age_effective_contact_matrix_arr[i][j] * states[indices['susceptible']][i][t] * (
                ...
                incidenceV[i][t] += (1 - a) * w * beta * age_effective_contact_matrix_arr[i][j] * states[indices['susceptible']][i][
            ...
            states[indices['exposedV']][i][t + 1] = states[indices['exposedV']][i][t] + incidenceV[i, t] - k * sigma * \
                                         states[indices['exposedV']][i][t]
            states[indices['asymptomaticN']][i][t + 1] = states[indices['asymptomaticN']][i][t] + (1 - b1) * sigma * states[indices['exposedN']][i][
                t] - allocation_rate * states[indices['asymptomaticN']][i][t]
            states[indices['asymptomaticV']][i][t + 1] = states[indices['asymptomaticV']][i][t] + (1 - b2) * k * sigma * states[indices['exposedV']][i][
                t] - allocation_rate * states[indices['asymptomaticV']][i][t]
            states[indices['infectedN']][i][t + 1] = states[indices['infectedN']][i][t] + b1 * sigma * states[indices['exposedN']][i][t] - alpha * \
                                          states[indices['infectedN']][i][t]
            states[indices['infectedV']][i][t + 1] = states[indices['infectedV']][i][t] + b2 * k * sigma * states[indices['exposedV']][i][t] - alpha * \
            ...
            states_increase[indices['asymptomaticN']][i][t] = (1 - b1) * sigma * states[indices['exposedN']][i][t]
            states_increase[indices['asymptomaticV']][i][t] = (1 - b2) * k * sigma * states[indices['exposedV']][i][t]
            states_increase[indices['infectedN']][i][t] = b1 * sigma * states[indices['exposedN']][i][t]
            states_increase[indices['infectedV']][i][t] = b2 * k * sigma * states[indices['exposedV']][i][t]
...
def get_R0_with_factor_vector(beta, a, w, k, b1, b2, sigma_inverse, alpha_inverse, gamma, susceptibility_factor_vector, num_agebrackets,
                              contact_matrix):
    """
...
states, incidenceN, incidenceV, indices, C = dou_seaiq_with_age_specific_contact_martix(contact_matrix_arr, ages, a, w, k,
                                                                                        beta,
                                                                                        susceptibility_factor_vector,
                                                                                        b1, b2, sigma_inverse,
                                                                                        alpha_inverse,
