...
def write_data(country, location, state, value, p, p_v, l, l_v, k, k_v, num_agebrackets=18, overwrite=True):
    ...
    file_path = os.path.join(output_source_dir, 'age-structured_output_source', 'l_k', file_name)
    e = os.path.exists(file_path)
    if e:
        print(f'{file_path} already exists.')
        if overwrite:
...
# %% 保存每日新增病例数
def write_per_day_new_cases(country, location, new_case_per_day, p, p_v, l, l_v, k, k_v, num_agebrackets=18,overwrite=True):
# def write_per_day_new_cases(country, location, new_case_per_day, p, p_v, num_agebrackets=18,overwrite=True):
...
timesteps = 100  # how long to run the SEAIQ model
...
for i in range(11):
    ...
    for j in range(11):
        ...
        # result_file_path = write_data(country, location, state, np.round(all_states.T), p, p_v, l, l_v, k, k_v,
        #                               num_agebrackets,
        #                               overwrite=True)
        new_case_per_day = np.round(np.sum(states_increase, axis=1).T)
        write_per_day_new_cases(country,location, new_case_per_day, p, p_v, l, l_v, k, k_v, num_agebrackets)
        # for age in np.arange(num_agebrackets):
        #     write_data_bracket_age(location, state, states[:, age, :].T, age, p, p_v,l,l_v,k,k_v,num_agebrackets=18)
