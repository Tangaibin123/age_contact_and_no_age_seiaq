...
# %% 将得到的结果读入csv文件中
def write_data(country, location, state, value, overwrite=True):
    file_name = f"{country}_{location}_{state}_optimal_vaccine_allocation.csv"
...
def write_per_day_new_cases(country, location, new_case_per_day,
                            overwrite=True):
        ...
        for i in range(num_agebrackets):
            for j in range(num_agebrackets):
                for kk, setting in enumerate(settings):
                    incidenceN_by_setting[kk][i][t] += p[i] * beta * weights[setting] * \
                    ...
                    incidenceV_by_setting[kk][i][t] += (1 - p[i]) * w * beta * weights[setting] * \
                                                       age_effective_contact_matrix_dic[setting][i][j] * \
                                                       states[indices['susceptible']][i][t] * (
                                                               states[indices['infectedN']][j][t] +
                                                               states[indices['infectedV']][j][t] +
...
# %%保存总计各个年龄段的累计发病率(没写完）
def write_daily_all_cumulative_incidence(country, location, cumulative_incidence, p, p_v, l, l_v, k, k_v,
                                         num_agebrackets=18,
                                         overwrite=True):
...
Baseline_matrix = combine_synthetic_matrices(contact_matrix_dic, weights, num_agebrackets)
...
# for i in ages:
#     for j in ages:
#         # print(type(Baseline_matrix[i][j]))
#         R[i][j] = Baseline_matrix[i][j] * (beta * k_v / gamma) * \
#                   ((1 - q_v) + k_v * q_v)
#     print(R[i].sum())


def vaccine_allocation(ages, p, matrix, beta, k, gamma_inverse, q, w, l, units=1000):
    R = np.zeros((18, 18))
    dR = np.zeros((18, 18))
    Ri = np.zeros(18)
    R_new = np.zeros((18, 18))
    gamma = 1 / gamma_inverse
    total_population = sum(ages.values())
    vaccine_num = (total_population * 0.6 / units)
    p_new = np.zeros(18)
    sigma_matrix = 0
    # print(vaccine_num)
    # print(ages)
    ...
    for x in range(units):
        ...
        for i in ages:
            for j in ages:
                for x in ages:
                    sigma_matrix += matrix[i][x]
                R[i][j] = sigma_matrix * (beta / (k * gamma)) * \
                          (p[i] * ((1 - q) + k * q) +
                           (1 - p[i]) * w * ((1 - q * l) + k * q * l))
        for i in ages:
            ...
            for j in ages:
                for x in ages:
                    sigma_matrix += matrix[i][x]
                R_new[i][j] = sigma_matrix * (beta / (k * gamma)) * \
                              (p_new[i] * ((1 - q) + k * q) +
                               (1 - p_new[i]) * w * ((1 - q * l) + k * q * l))
                # dR[i][j] = pow(R[i][j], 0.5) - pow(R_new[i][j], 0.5)
                # dR[i][j] = pow((R[i][j] - R_new[i][j]), 0.5)
                dR[i][j] = R[i][j]-R_new[i][j]

            Ri[i] = dR[i].sum()
        # if (x == 1 or x == 500 or x==900):
        #     print('p\n', p)
        #     print("----------------------------")
        #     print('Ri\n', Ri)
        #     print("----------------------------#############--------------------------")
        max_index = np.argmax(Ri)  # 获取最大值索引
        # max = np.max(R0)
        p[max_index] = p_new[max_index]
        #
        # if(x==9 or x==500):
        #     print('p\n',p)
        #     print('R\n',Ri)
...
# p_v *=0.5
# 最优疫苗分配算法
p_v = vaccine_allocation(ages, p_v, Baseline_matrix, beta, k_v, gamma_inverse, q_v, w_v, l_v)

R0_list = []
attack = []
# p_v[4] = 0.9;p_v[5] = 0.9; p_v[6] = 0.9; p_v[10] = 0.9
...
# 写入每个年龄段的累计发病率
# write_cumulative_incidence(country, location, cumulative_incidence.T, p, 0.5, l, l_v, k, k_v)

# %%
all_states = np.round(np.sum(states, axis=1))
result_file_path = write_data(country, location, state, np.round(all_states.T),
                              overwrite=True)
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
write_per_day_new_cases(country, location, new_case_per_day, overwrite=True)
# ))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))
