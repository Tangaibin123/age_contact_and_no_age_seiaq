    ...
    for t in range(timesteps):
        for i in range(num_agebrackets):
            ...
            if (states[indices['susceptible']][i][t] - incidenceN_by_setting[:, i, t].sum() - incidenceV_by_setting[:,
                ...
                # 不这样的话当传播率过于大的时候会导致总感染人数超过实际人数
                states[indices['exposedN']][i][t + 1] = states[indices['exposedN']][i][t] + \
                                                        states[indices['susceptible']][i][t] - sigma * \
                                                        states[indices['exposedN']][i][t]
                states[indices['exposedV']][i][t + 1] = states[indices['exposedV']][i][t] + \
                                                        states[indices['susceptible']][i][t] - sigma * \
...
beta = 0.4  #
l_v = 0.5 #接种后有症状的比例
sigma_inverse = 2.0  # mean incubation period
k_v = 0.5
