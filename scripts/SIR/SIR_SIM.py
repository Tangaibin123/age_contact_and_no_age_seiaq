...
# %% 将得到的结果读入csv文件中
def write_data(state, value,  overwrite=True):

    file_name = f"SIR_{state}_numbers_no_age.csv"
    file_path = os.path.join(project_results_dir, 'no_age_structured', file_name)
...
# %% 保存每日新增病例数
def write_per_day_new_cases(new_case_per_day, overwrite=True):

    file_name = f"SIR_new_case_per_day_no_age.csv"
    file_path = os.path.join(project_results_dir, 'no_age_structured', file_name)
    e = os.path.exists(file_path)
    if e:
        print(f'{file_path} already exists.')
...
def write_cumulative_incidence(location, cumulative_incidence, num_agebrackets=18, overwrite=True):
    file_name = f"SIR_{location}_cumulative_incidence.csv"
    file_path = os.path.join(project_results_dir, 'no_age_structured', file_name)
    e = os.path.exists(file_path)
    if e:
        print(f'{file_path} already exists.')
...
# %% 模型的模拟
def simulation_with_no_age_structured_model(total_population, p, w, beta, q, l, sigma_inverse, k, gamma_inverse,
    ...
    for t in range(timesteps):
        ...
        states_increase[indices['quarantined']][t+1] = gamma * (
                    states[indices['infectedV']][t] + states[indices['infectedN']][t]) + k * gamma * (
                                                                 states[indices['asymptomaticN']][t] +
                                                                 states[indices['asymptomaticV']][t])
        cumulative_incidence[t] = (states_increase[indices['asymptomaticN']].sum() + states_increase[indices['asymptomaticV']].sum() + states_increase[indices['infectedN']].sum() + states_increase[indices['infectedV']].sum()) / total_population * 100
...
l_v = 0.8  # 0-1
k_v = 0.8  # 0-1
sigma_inverse = 1  # mean latent period
...
#控制更改的三个参数
total_population = 500000   #模拟人数
gamma_inverse = 7.0  # 感染者平均恢复时间
beta = 0.5  #病毒传播率，每个感染者每天可以感染的人数
timesteps = 150  # how long to run the  model

states, incidenceN, incidenceV, indices, state_increase, cumulative_incidence = simulation_with_no_age_structured_model(total_population, p_v, w_v, beta,
                                                                                                                                q_v, l_v,
                                                                                                                                sigma_inverse, k_v,
                                                                                                                                gamma_inverse,
...
infected_rate = total_infectedN/total_population
print('总感染率:',infected_rate)
# print(S[200:250])
# print(exposed[200:250])
day = [i for i in range(151)]
...
# 将每次运行的结果存起来
result_file_path = write_data(state, np.round(all_states.T),  overwrite=True)
...
write_per_day_new_cases(new_case_per_day, overwrite=True)
write_cumulative_incidence(location, cumulative_incidence.T)
