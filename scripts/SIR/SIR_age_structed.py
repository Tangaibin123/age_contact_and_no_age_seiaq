...
# path to project_results
project_results_dir = os.path.join(maindir, 'project_results')
...
# %% 将得到的结果读入csv文件中
def write_data( location, state, value,  num_agebrackets=18, overwrite=True):
    file_name = f"SIR_{location}_{state}_numbers_{num_agebrackets}.csv"
    file_path = os.path.join(project_results_dir, 'age_structured', file_name)
    e = os.path.exists(file_path)
    if e:
        print(f'{file_path} already exists.')
        if overwrite:
...
def write_per_day_new_cases(location, new_case_per_day, num_agebrackets=18,
                            overwrite=True):
    ...
    file_name = f"SIR_{location}_new_case_per_day_{num_agebrackets}.csv"
    file_path = os.path.join(project_results_dir, 'age_structured', file_name)
    e = os.path.exists(file_path)
    if e:
        print(f'{file_path} already exists.')
...
# %% 模型的模拟
def dou_seaiq_with_age_specific_contact_martix(contact_matrix_dic, weights, ages, p, w, beta, q, l, k,
    ...
    cumulative_incidence_all = np.zeros(timesteps)
    ...
    for t in range(timesteps):
        for i in range(num_agebrackets):
            ...
            if (states[indices['susceptible']][i][t] - incidenceN_by_setting[:, i, t].sum() - incidenceV_by_setting[:,
                ...
                # incidenceN_by_setting[:, i, t].sum() = states[indices['susceptible']][i][t]
                # 不光明天所剩的S变为0，今天要转变为E或者I的S也只能是states[indices['susceptible']][i][t]而不是incidenceN_by_setting[:, i, t].sum()
                # 当传播率过于大的时候会导致总感染人数超过实际人数
            else:
            ...
            states_increase[indices['asymptomaticV']][i][t + 1] = (1 - q * l) * sigma * states[indices['exposedV']][i][t]
            ...
            states_increase[indices['quarantined']][i][t + 1] = gamma * (
                    states[indices['infectedV']][i][t] + states[indices['infectedN']][i][t]) + k * gamma * (
                                                                        states[indices['asymptomaticN']][i][t] +
                                                                        states[indices['asymptomaticV']][i][t])
            cumulative_incidence[i][t] = (
                                          states_increase[indices['infectedN']][i].sum()
                                          ) / ages[i] * 100
            cumulative_incidence = np.array(cumulative_incidence)
            cumulative_incidence[cumulative_incidence>100] = 100
        cumulative_incidence_all[t] = states_increase[3:7].sum() / total_population * 100
        # cumulative_all_group[t] = cumulative_incidence[t].sum

    return states, incidenceN_by_setting, incidenceV_by_setting, states_increase, cumulative_incidence,cumulative_incidence_all, indices, setting_indices
...
#保存发病率
def write_cumulative_incidence(location, cumulative_incidence, num_agebrackets=18, overwrite=True):
    file_name = f"SIR_{location}_cumulative_incidence_{num_agebrackets}_all.csv"
    file_path = os.path.join(project_results_dir, 'age_structured', file_name)
    e = os.path.exists(file_path)
    if e:
        print(f'{file_path} already exists.')
        if overwrite:
            ...
            for v in range(len(cumulative_incidence)):
                f.write(
                    f"{v:.2f},{cumulative_incidence[v]:.2f}\n")
            f.close()
        else:
            print('Not overwriting the existing file.')
    else:
...
# %% 保存每个年龄段的累计发病率
def write_cumulative_incidence_age( location, cumulative_incidence,num_agebrackets=18,overwrite=True):
    file_name = f"SIR_{location}_cumulative_incidence_{num_agebrackets}.csv"
    file_path = os.path.join(project_results_dir, 'age_structured',file_name)
    e = os.path.exists(file_path)
    if e:
        print(f'{file_path} already exists.')
...
q_v = 1
# param = 'gamma_inverse'
p_v = 1  # 0.1 0.5 0.9  # percent of unvaccinated
w_v = 0.5
l_v = 0.5 #接种后有症状的比例

k_v = 0.5
sigma_inverse = 1.0  # mean incubation period
...
states, incidenceN, incidenceV, states_increase, cumulative_incidence,cumulative_incidence_all, indices, setting_indices = dou_seaiq_with_age_specific_contact_martix(
    contact_matrix_dic, weights, ages, p_v, w_v, beta, q_v, l_v, k_v, susceptibility_factor_vector,
    sigma_inverse, gamma_inverse, initial_infected_age, percent_of_initial_infected_seeds,
    num_agebrackets, timesteps)


# 写入每个年龄段的累计发病率
write_cumulative_incidence_age( location, cumulative_incidence.T)
write_cumulative_incidence(location, cumulative_incidence_all.T)


##写每个状态的仿真
all_states = np.round(np.sum(states, axis=1))
result_file_path = write_data( location, state, np.round(all_states.T), num_agebrackets,
                              overwrite=True)

# 将每日增长的病例保存起来
new_case_per_day = np.round(np.sum(states_increase, axis=1).T)
write_per_day_new_cases( location, new_case_per_day,num_agebrackets, overwrite=True)
