...
def plot_per_day_new_cases(country, location, p, p_v, num_agebrackets, isSave=False):
    ...
    plt.suptitle('avg allocation  new increase with age-structured')
    plt.subplots_adjust(left=0.1, right=0.95, bottom=0.1, top=0.9, wspace=0.1, hspace=0.4)

    ax1[0].set_title('symptomatic and no-vaccine')
    # ax1[1].set_ylabel('count')
    ax1[0].plot(per_day_new_cases[indices['infectedN']])
    max = np.max(per_day_new_cases[indices['infectedN']])
    ax1[0].axhline(y=max, color='r', linestyle='--')
    ax1[0].text(0, max + 500, str(max), color='r')

    ax1[1].set_title('asymptomatic and vaccinated')
    ax1[1].plot(per_day_new_cases[indices['asymptomaticV']])
    ...
    ax2[0].set_title('symptomatic and vaccinated')
    # ax2[1].set_ylabel('count'1
    ax2[0].plot(per_day_new_cases[indices['infectedV']])
    max = np.max(per_day_new_cases[indices['infectedV']])
    ax2[0].axhline(y=max, color='r', linestyle='--')
    ax2[0].text(0, max + 500, str(max), color='r')

    ax2[1].set_title('total infected')
    ax2[1].set_xlabel('day')
    ax2[1].plot(per_day_new_cases[indices['asymptomaticV']] + per_day_new_cases[indices['asymptomaticN']]+
    ...
    ax2[1].text(0, max - 5000, str(max), color='r')
...
def plot_per_day_new_cases_single(country, location, p, p_v, num_agebrackets, isSave=False):
    # file_name = f"{country}_{location}_per_day_new_increase_infectious_numbers_{num_agebrackets}.csv"
    # file_name = f"China_Shanghai_new_case_per_day_18_p=0.50_INP.csv"
    # file_name = f"{country}_{location}_new_case_per_day_{num_agebrackets}_{p}={p_v:.2f}_NPI.csv"
    # file_name = f"{country}_{location}_new_case_per_day_allocation"
    ...
    ax.set_xlabel('day')
    ax.plot(per_day_new_cases[indices['asymptomaticV']] + per_day_new_cases[indices['asymptomaticN']]+
                per_day_new_cases[indices['infectedN']] + per_day_new_cases[indices['infectedV']])
    max = np.max(per_day_new_cases[indices['asymptomaticV']] + per_day_new_cases[indices['asymptomaticN']]+
                per_day_new_cases[indices['infectedN']] + per_day_new_cases[indices['infectedV']])
    ax.axhline(y=max, color='r', linestyle='--')
    ax.text(0, max - 5000, str(max), color='r')
...
plot_per_day_new_cases_single(country, location, p, p_v, num_agebrackets)
